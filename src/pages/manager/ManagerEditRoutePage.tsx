...
export default function ManagerEditRoutePage() {
  ...
  const [selectedVendorIds, setSelectedVendorIds] = useState<string[]>(route?.vendorIds || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(false);
    navigate(`/manager/routes/${route.id}`);
  };

  return (
    <MobileLayout
      header={<PageHeader title="Edit Route" subtitle={tenant?.name} showBack showLogout />}
    >
      <div className="p-4">
